...
class ProjectProject(models.Model):
    ...
    budget_ids = fields.One2many(
        comodel_name="crossovered.budget", inverse_name="project_id", string="Budgets"
    )
    ...
    def _compute_budget_count(self):
        ...
        for record in self:
            record.budget_count = len(record.with_context(active_test=False).budget_ids)
            record.current_budget_id = record.budget_ids.filtered(
                lambda b: from_string(b.budget_date) >= month_start
                and from_string(b.budget_date) <= month_end
            )[:1]
    ...
    def create_initial_project_budget(self, date=False):
        ...
        for record in self.filtered(
            ...
            budget = budget_obj.create(
                {
                    "name": _(u"Initial {} budget: {}").format(
                        budget_date.year, record.name
                    ),
                    "initial": True,
