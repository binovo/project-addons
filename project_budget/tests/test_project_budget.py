...
class TestProjectBudget(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        analytic_account = cls.env["account.analytic.account"].create(
            {
                "name": "Analytic Account for Project",
                "code": "AN001",
                "plan_id": plan.id,
            }
        )
        cls.project = cls.project_model.create(
            {"name": "New Project", "analytic_account_id": analytic_account.id}
        )

    def test_project_creation(self):
        self.assertTrue(self.project.budget_ids)
        self.assertTrue(self.project.budget_ids[:1].initial)
        self.assertEqual(len(self.project.budget_ids), self.project.budget_count)
        self.assertTrue(self.project.has_current_budget)
        self.assertIn(
            self.project, self.project.search([("has_current_budget", "=", True)])
        )
        self.assertEqual(
            len(self.project.mapped("budget_ids.crossovered_budget_line")), 26
        )
    ...
    def test_budget_line_creation(self):
        ...
        new_budget = self.project.budget_ids.copy(
            default={"crossovered_budget_line": []}
        )
        new_budget.date_to = from_string(fields.Date.today()).replace(month=12, day=29)
        new_budget.button_compute_lines()
        self.assertEqual(len(new_budget.crossovered_budget_line), 26)

    def test_initial_budget_duplication(self):
        old_budget = self.project.budget_ids[:1]
        self.assertFalse(old_budget.crossovered_budget_line[:1].initial_budget_line_id)
        old_budget.crossovered_budget_line[:1].write({"planned_amount": 50.0})
        self.assertEqual(
        ...
        new_budget.crossovered_budget_line[:1].write({"planned_amount": 150.0})
        self.assertNotEqual(
    ...
    def test_initial_budget_wizard(self):
        ...
        self.assertEqual(len(self.project.budget_ids), 1)
        wizard = self.wizard.with_context(active_ids=[self.project.id]).create(
    ...
    def test_search_project_budget(self):
        ...
        self.assertIn(("id", "not in", self.project.ids), action_dict.get("domain"))
        action_dict = wizard.search_project_in_budget()
        self.assertIn(("id", "in", self.project.ids), action_dict.get("domain"))
