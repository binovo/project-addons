...
class ProjectProject(models.Model):
    ...
    def _compute_planned_dates(self):
        for project in self:
            if project.planned_date_start and project.planned_date_end:
    ...
    def _compute_task_dates(self):
        ...
        for project in self.filtered('task_ids'):
            ...
            if project.date_start:
                project.task_date_start = project.date_start
            end_dates = (
            ...
            project.task_date_margin = relativedelta2months(
                relativedelta.relativedelta(
                    ...
                    str2datetime(project.date_start)))
...
class ProjectMember(models.Model):
    ...
    def _compute_project_planned_hours(self):
        for record in self:
            if record.planned_hours_percentage:
                record.project_planned_hours = (
                    record.project_id.planned_hours *
                    (record.planned_hours_percentage / 100.0))
            else:
                record.project_planned_hours = record.project_id.planned_hours
    ...
    def _compute_task_planned_hours(self):
        for record in self:
            if record.planned_hours_percentage:
                ...
                record.task_planned_hours = record.project_id.task_planned_hours
