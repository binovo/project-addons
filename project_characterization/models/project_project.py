...
class ProjectProject(models.Model):
    ...
    nonoperative = fields.Boolean(related="analytic_account_id.nonoperative", readonly=False, store=True)
    num_code = fields.Char(related="analytic_account_id.num_code", readonly=False)
    op_space_id = fields.Many2one(related="analytic_account_id.op_space_id", readonly=False, store=True)
    res_area_id = fields.Many2one(related="analytic_account_id.res_area_id", readonly=False, store=True)
    res_area_type_id = fields.Many2one(related="analytic_account_id.res_area_type_id", readonly=False, store=True)
    res_character_id = fields.Many2one(related="analytic_account_id.res_character_id", readonly=False, store=True)
    res_target_id = fields.Many2one(related="analytic_account_id.res_target_id", readonly=False, store=True)
    res_team_id = fields.Many2one(related="analytic_account_id.res_team_id", readonly=False, store=True)
