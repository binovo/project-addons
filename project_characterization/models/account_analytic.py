...
class AccountAnalyticAccount(models.Model):
    ...
    res_area_id = fields.Many2one(comodel_name="res.area", string="Area")
    op_space_id = fields.Many2one(
        comodel_name="res.opportunity.space", string="Opportunity Space"
    )
    res_character_id = fields.Many2one(comodel_name="res.character", string="Character")
    ...
    res_team_id = fields.Many2one(comodel_name="res.team", string="Team")
    res_target_id = fields.Many2one(comodel_name="res.target", string="Target")
...
class AccountAnalyticLine(models.Model):
    ...
    nonoperative = fields.Boolean(
        string="Non Operative",
        related="account_id.nonoperative",
        store=True,
        compute_sudo=True,
    )
